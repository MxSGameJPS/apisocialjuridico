...
export async function plataformaMonitoramentoRoutes(app) {
  ...
  app.post('/api/plataformas/monitoramentos/listar', { preHandler: internalAuth }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ success: false, message: 'Dados inválidos.', errors: parsed.error.flatten().fieldErrors });
    const data = await listarMonitoramentosPlataforma({ ownerRef: parsed.data.owner_ref, ativo: parsed.data.ativo, limite: parsed.data.limite });
  ...
  app.post('/api/plataformas/monitoramentos/executar', { preHandler: internalAuth }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ success: false, message: 'Dados inválidos.', errors: parsed.error.flatten().fieldErrors });
    const data = await executarMonitoramentosPlataforma({ ownerRef: parsed.data.owner_ref, monitoramentoId: parsed.data.monitoramento_id, limiteMonitoramentos: parsed.data.limite_monitoramentos, limitePorMonitoramento: parsed.data.limite_por_monitoramento });
  ...
  app.post('/api/plataformas/eventos', { preHandler: internalAuth }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ success: false, message: 'Dados inválidos.', errors: parsed.error.flatten().fieldErrors });
    const data = await listarEventosPlataforma({ ownerRef: parsed.data.owner_ref, monitoramentoId: parsed.data.monitoramento_id, lido: parsed.data.lido, limite: parsed.data.limite });
